...
readonly LITE_NAS_POSTFIX_NORMAL_CONFIG_DIR="${LITE_NAS_POSTFIX_NORMAL_CONFIG_DIR:-$PACKAGE_ROOT/configs/etc/postfix}"
readonly LITE_NAS_POSTFIX_TEST_CONFIG_DIR="${LITE_NAS_POSTFIX_TEST_CONFIG_DIR:-$PACKAGE_ROOT/configs/etc/postfix/testing}"
readonly LITE_NAS_POSTFIX_CAPTURE_HELPER_SOURCE="${LITE_NAS_POSTFIX_CAPTURE_HELPER_SOURCE:-$PACKAGE_ROOT/scripts/postfix/postfix-test-pipe.sh}"
readonly LITE_NAS_POSTFIX_CAPTURE_HELPER_TARGET="${LITE_NAS_POSTFIX_CAPTURE_HELPER_TARGET:-/usr/libexec/lite-nas/postfix-test-pipe}"
...
install_postfix_config() {
	...
	install -m 0644 "$source_dir/main.cf" "$LITE_NAS_POSTFIX_MAIN_CF_TARGET"
	install -m 0644 "$source_dir/master.cf" "$LITE_NAS_POSTFIX_MASTER_CF_TARGET"
}

install_capture_helper() {
	local target_dir

	if [ ! -f "$LITE_NAS_POSTFIX_CAPTURE_HELPER_SOURCE" ]; then
		log.error "Missing capture helper source: $LITE_NAS_POSTFIX_CAPTURE_HELPER_SOURCE"
		exit 1
	fi

	target_dir="$(dirname "$LITE_NAS_POSTFIX_CAPTURE_HELPER_TARGET")"
	install -d -m 0755 "$target_dir"
	install -m 0755 "$LITE_NAS_POSTFIX_CAPTURE_HELPER_SOURCE" "$LITE_NAS_POSTFIX_CAPTURE_HELPER_TARGET"
}

prepare_capture_file() {
	local capture_dir

	capture_dir="$(dirname "$LITE_NAS_POSTFIX_CAPTURE_FILE")"
	...
	chown root:root "$LITE_NAS_POSTFIX_CAPTURE_FILE"
	chmod 0644 "$LITE_NAS_POSTFIX_CAPTURE_FILE"

	rm -rf "$LITE_NAS_POSTFIX_CAPTURE_LOCK_DIR"
}
...
if [ "$mode" = "capture" ]; then
	...
	install_postfix_config "$LITE_NAS_POSTFIX_TEST_CONFIG_DIR"
	install_capture_helper
	prepare_capture_file
	validate_postfix_config
	restart_postfix
	log.popTask
	log.info "Postfix capture mode enabled. Mail is appended to $LITE_NAS_POSTFIX_CAPTURE_FILE"
	exit 0
fi

log.pushTask "Restoring normal Postfix mode"
install_postfix_config "$LITE_NAS_POSTFIX_NORMAL_CONFIG_DIR"
