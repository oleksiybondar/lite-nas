...
//   - occurrences are indexed by event_rec_id only (no timestamp index).
...
func BuildSchemaMigrationQueries() []Query {
	return []Query{
		...
		{SQL: "CREATE TABLE IF NOT EXISTS occurrences (rec_id INTEGER PRIMARY KEY AUTOINCREMENT, event_rec_id INTEGER NOT NULL, ts TEXT NOT NULL, value_type TEXT NOT NULL CHECK(value_type IN ('int', 'float', 'text', 'bool')), value_num REAL, value_text TEXT, value_bool INTEGER CHECK(value_bool IN (0, 1)), value_unit TEXT)"},
