...
func loadConfigFromFixture(t *testing.T) (Config, error) {
	...
	configData := []byte(
		"[messaging]\n" +
			...
			"cert=/etc/lite-nas/certificates/transport/lite-nas-system-metrics-cli/client.crt\n" +
			"key=/etc/lite-nas/certificates/transport/lite-nas-system-metrics-cli/client.key\n" +
			"[logging]\n" +
			"level=info\n" +
			"format=rfc5424\n" +
			"output=stderr\n",
