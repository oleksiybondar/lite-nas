...
Options:
  --arch=amd64|arm64              Build target architecture when binary paths are not set.
...
tmp_dir=""
if [ -z "$system_metrics_binary" ] || [ -z "$system_metrics_cli_binary" ] || [ -z "$web_gateway_binary" ]; then
	tmp_dir="$(mktemp -d)"
