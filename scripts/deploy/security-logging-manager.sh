...
readonly LITE_NAS_SECURITY_LOGGING_MANAGER_RUNTIME_USER="${LITE_NAS_SECURITY_LOGGING_MANAGER_RUNTIME_USER:-lite-nas-security-logging-manager}"
...
deploy.securityLoggingManager.installConfig() {
	...
	install -m 0640 -o root -g "$LITE_NAS_SECURITY_LOGGING_MANAGER_CONFIG_GROUP" \
...
deploy.securityLoggingManager.enableAndStart() {
	...
	systemctl enable --now "$LITE_NAS_SECURITY_LOGGING_MANAGER_SERVICE_NAME.service"
}
