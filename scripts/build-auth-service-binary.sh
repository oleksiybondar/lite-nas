...
output_path="$(build.prepareOutputPath "$output_path" "auth-service" "auth-service")"

output_dir="$(dirname "$output_path")"
mkdir -p "$output_dir"

build.prepareGoCache
build.goBinary "auth-service" "$LITE_NAS_AUTH_SERVICE_MODULE" "$output_path" 1 "$target_arch"
