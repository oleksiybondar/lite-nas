...
readonly LITE_NAS_NGINX_DEFAULT_SITE_SYMLINK="${LITE_NAS_NGINX_DEFAULT_SITE_SYMLINK:-$LITE_NAS_NGINX_SITES_ENABLED_DIR/default}"
...
deploy.nginx.enableSite() {
	...
	if [ -L "$LITE_NAS_NGINX_DEFAULT_SITE_SYMLINK" ]; then
		rm -f "$LITE_NAS_NGINX_DEFAULT_SITE_SYMLINK"
	fi
}
