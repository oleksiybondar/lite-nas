...
output_path="$(build.prepareOutputPath "$output_path" "web-gateway" "web-gateway")"

output_dir="$(dirname "$output_path")"
mkdir -p "$output_dir"

build.prepareGoCache
build.goBinary "web-gateway" "$LITE_NAS_WEB_GATEWAY_MODULE" "$output_path" 0 "$target_arch"
