...
read -r -a certificate_users <<<"${LITE_NAS_NATS_CERT_USERS:-lite-nas-system-metrics lite-nas-network-metrics lite-nas-process-metrics lite-nas-disk-metrics lite-nas-service-metrics lite-nas-zfs-metrics lite-nas-zfs-metrics-cli lite-nas-system-metrics-cli lite-nas-network-metrics-cli lite-nas-sys-log-mgr lite-nas-sec-log-mgr lite-nas-sys-log-mgr-cli lite-nas-sec-log-mgr-cli lite-nas-sys-email-notifier lite-nas-sec-email-notifier lite-nas-web-gateway lite-nas-auth-service lite-nas-resources-monitor lite-nas-rbac-service}"
...
Options:
  ...
  --if-missing Rotate only when one or more expected certificates are missing.
...
for tool in openssl getent grep groupadd chown chmod find install; do
...
client_certificate_matches_identity() {
	...
	if openssl x509 -in "$certificate_file" -noout -ext subjectAltName 2>/dev/null | grep -F "DNS:$certificate_user" >/dev/null 2>&1; then
		return 0
	fi

	log.info "Client certificate is missing expected SAN mapping: $certificate_user"
	return 1
}

required_certificates_are_current() {
	local certificate_user
	local required_files=(
		"$nats_certificate_dir/root-ca.key"
	...
	for required_file in "${required_files[@]}"; do
		if [ ! -f "$required_file" ]; then
			log.info "Missing certificate asset: $required_file"
			return 1
		fi
	done

	for certificate_user in "${certificate_users[@]}"; do
		if ! client_certificate_matches_identity "$certificate_user"; then
			return 1
		fi
	done
...
subjectAltName = DNS:${ENV::LITE_NAS_CERTIFICATE_USER}
EOF
...
if [ "$rotate_only_if_missing" -eq 1 ] && required_certificates_are_current; then
	log.info "All expected NATS certificates already exist and match the current LiteNAS identity format; skipping rotation."
	exit 0
fi
