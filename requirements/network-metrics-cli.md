# Network Metrics CLI — Requirements

## Overview

The Network Metrics CLI is a read-only command-line client for the Network
Metrics service. It requests either the current network metrics snapshot or the
recent snapshot history and renders the result for terminal use.

The CLI is intended for direct operator usage on a LiteNAS host. By default it
presents the current snapshot in a human-readable format instead of raw JSON.
...
The CLI MUST request the current network metrics snapshot when it is invoked
without a history flag.
...
- Request for the current network metrics snapshot
...
- Running the CLI with any supported section flag requests the current snapshot
- The current snapshot mode is the default execution mode
...
When rendering the current network metrics snapshot, the CLI MUST print a
human-readable terminal format instead of JSON.

#### FR-002 Input

- Current network metrics snapshot from FR-001

#### FR-002 Output

- Formatted terminal output containing one or more selected sections

#### FR-002 Acceptance Criteria

- Default output includes interfaces, protocols, sockets, and pressure
  sections
- Output is formatted for direct terminal reading rather than JSON parsing
- The current snapshot timestamp is not printed in the default human-readable
  output
...
operators can request only selected snapshot sections.
...
- Human-readable output containing only the selected sections

#### FR-003 Acceptance Criteria

- `--interfaces` prints only the interfaces section
- `--protocols` prints only the protocols section
- `--sockets` prints only the sockets section
- `--pressure` prints only the pressure section
- Providing multiple section flags prints the selected sections in a stable
  order separated by a visual separator
- When no section flags are provided, all sections are printed
...
- Running the CLI with `--history` requests metrics history instead of the
  current snapshot
...
The CLI MUST expose a flag-based interface for selecting snapshot mode, history
mode, and current-snapshot section filters.
...
- The CLI accepts `--interfaces`, `--protocols`, `--sockets`, and `--pressure`
  as current snapshot filters
...
The CLI MUST write successful command results to standard output in the format
...
configuration, connect to the messaging system, or retrieve the requested data.
...
The CLI MUST support selecting a specific configuration file path at runtime.
...
- Configuration path selection does not change command behavior apart from which
