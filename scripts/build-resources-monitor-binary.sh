...
usage() {
	cat <<'MSG'
Usage: scripts/build-resources-monitor-binary.sh [options]

Options:
  --output PATH       Output binary path. Defaults to .build/resources-monitor/linux-<arch>/resources-monitor
...
output_path="$(build.prepareOutputPath "$output_path" "resources-monitor" "resources-monitor")"

output_dir="$(dirname "$output_path")"
mkdir -p "$output_dir"

build.prepareGoCache
build.goBinary "resources-monitor" "$LITE_NAS_RESOURCES_MONITOR_MODULE" "$output_path" 0 "$target_arch"
