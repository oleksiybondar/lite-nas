...
usage() {
	cat <<'MSG'
Usage: scripts/build-zfs-metrics-cli-binary.sh [options]

Options:
  --output PATH       Output binary path. Defaults to .build/zfs-metrics-cli/linux-<arch>/zfs-metrics-cli
...
output_path="$(build.prepareOutputPath "$output_path" "zfs-metrics-cli" "zfs-metrics-cli")"

output_dir="$(dirname "$output_path")"
mkdir -p "$output_dir"

build.prepareGoCache
build.goBinary "zfs-metrics-cli" "$LITE_NAS_ZFS_METRICS_CLI_APP_MODULE" "$output_path" 0 "$target_arch"
